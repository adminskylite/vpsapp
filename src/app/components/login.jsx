...
export default function LoginForm() {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    // TODO: wire up to your login API
    console.log(form);
    router.push("/auth");
  }
  ...
  return (
    <main className="h-screen overflow-y-auto flex items-center justify-center bg-linear-to-br from-[#7C3AED] via-[#6C3EF4] to-[#5126D6] p-4">
      <section className="relative w-full max-w-[390px] min-h-full max-h-[820px] overflow-hidden rounded-[10px] shadow-[0_30px_80px_rgba(0,0,0,.25)] flex flex-col bg-linear-to-b from-[#6C3EF4] to-[#5126D6]">
        ...
        <form
          onSubmit={handleSubmit}
          className="relative flex-1 min-h-0 rounded-t-[24px] bg-white px-6 pt-8 pb-6 flex flex-col overflow-y-auto"
        >
          <label className="text-xs font-semibold text-slate-600 mb-1.5">
            Email
          </label>
          ...
          <button
            ...
            className="inline-flex h-[52px] w-full items-center justify-center gap-1.5 rounded-[18px] bg-linear-to-r from-[#7C3AED] to-[#5126D6] text-base font-bold text-white shadow-[0_15px_35px_rgba(108,62,244,.35)] transition-all duration-300 hover:scale-[1.02] active:scale-100 cursor-pointer"
          >
            Login
            <ArrowRight size={20} strokeWidth={3} />
          </button>
